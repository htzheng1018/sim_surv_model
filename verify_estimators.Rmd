...
Bootstrap function
```{r}
boot_ci = function(model, wt, data, t) {
  nn = nrow(data)
  R = 1000
  surv_cox.boot = c()
  surv_two.boot = c()
  ...
  result = data.frame(cox_low = ci_cox.boot[1], cox_up = ci_cox.boot[2],
...
run_on_cluster(
  ...
  first = {
    ...
    sim %<>% set_script(function() {
      ...
      model_two = coxph(Surv(Y, delta) ~ X1 + X2 + S, data = dat_phaseTwo)
      ...
      true = c()
      for (i in 1: time_max) {
        true[i] = surv_true(L$surv_time$surv_type, L$surv_time$surv_params, model_one, i, dat_phaseOne)
      }
      t = which.min(abs(true - 0.5))
      
      # bootstrap
      surv_ci = boot_ci(model_two, wt_phase, dat_phaseTwo, t)
