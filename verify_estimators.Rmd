...
```{r}
dat_phaseOne = create_data(1000, "Gompertz", c(0.2138, 7e-8))
...
```{r}
model = coxph(Surv(Y, delta) ~ X1 + X2 + S, data = dat_phaseOne, weights = ipw)
```


true lambda
```{r}
lambda_true = basehaz(model, dat_phaseOne, centered = F)
```
...
```{r}
# estimate beta
beta_init = matrix(c(0.011, 0.003), 2, 1)
...
score_function = function(beta, data) {
  ...
  score = matrix(0, nrow(beta_matrix), 1)
  for (i in 1: n) {
    nom = 0
    for (j in 1: n) {
      R = I(data$Y[j] >= data$Y[i])
      nom = nom + R * X[j, ] %*% exp(t(beta_matrix) %*% X[j, ])
    }
    #print(nom)
    den = 0
    for (j in 1: n) {
      R = I(data$Y[j] >= data$Y[i])
      den = den + R * exp(t(beta_matrix) %*% X[j, ])
    }
    score = score + delta[i] * (X[i, ] - nom/den[1, 1])
  }
  return(score)
}


score_function(beta_init, dat_phaseTwo)


result = optim(par = as.vector(beta_init),
               fn = score_function,
               data = dat_phaseTwo,
               method = "BFGS")

theta_opt <- matrix(result$par, 2, 2)
```
