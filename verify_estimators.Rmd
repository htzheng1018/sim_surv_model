...
```{r}
model_real = coxph(Surv(Y, delta) ~ X1 + X2 + S, data = dat_phaseOne)
model_est = coxph(Surv(Y, delta) ~ X1 + X2 + S, data = dat_phaseTwo)
...
```{r}
surv_est = function(model, t, wt_phase, data) {
  bh_1 = basehaz(model)
  ...
  proportional = exp(beta %*% t(XS))
  
  result = exp(- lambda_0 * wt_phase * proportional)
  return(mean(result))
}
```
