...
This file simulates data of Cox PH model using a single core.
```{r}
sim = new_sim()
```
...
sim %<>% set_levels(
  n = 58721,
  surv_time = list(
    "Exp" = list(surv_type = "Exponential", surv_params = 1.5e-2), # there are some problems with lambda in exponential distribution
...
```{r}
sim %<>% set_config(num_sim = 1000, parallel = FALSE, seed = 1018)
```
